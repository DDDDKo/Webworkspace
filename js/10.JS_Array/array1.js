...
/*
    ...
    - 자바스크립트 배열은 각 요소의 데이터 타입이 서로 달라도 됨
*/
const emptyArray = [];
...
/*
    배열의 요소 추가
    - 배열[인덱스] = 값 : 요소 추가 가능, 순서를 생각할 필요는 없음 (여기서의 인덱스는 추가할 인덱스 번호)
    - 뛰어 넘은 인덱스의 값에는 empty item이 추가 됨
*/
const emptyArray2 = [];
emptyArray2[2] = 10;
...
/*
    - for-in 순회 방식은 index를 사용하여 한번 더 작업해야 하기 때문에 요소를 읽어 들이는 순회에는 적합하지 않을 수 있다
...
/*
    push(element) :
    - 배열의 마지막 인덱스에 요소를 추가를 하고 변경된 길이를 반환
*/
...
/*
    ...
    - 매개변수로 전달된 값들을 해당 배열에 마지막 요소 뒤에 연결하여 새로운 배열을 반환
*/
...
/*
    slice(start, end) : 
    - start에 해당하는 인덱스부터 end에 해당하는 인덱스 값까지 복사하여 반환
*/
console.log(newFruits.slice(1,3));
console.log(newFruits.slice);
