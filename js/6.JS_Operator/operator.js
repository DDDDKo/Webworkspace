...
// 선 대입 후 증가
result = single++ * 8;  // result = 0 , single = 1
// 선 증가 후 대입
result = ++single * 8; // result = 16, single = 2
// 선 대입 후 감소
result = single-- + 9  - ++single; // result = 9, single = 2
// 선 감소 후 대입
result = --single;
...
/*
    비교 연산자 : 좌항과 우항이 같은지 비교하여 논리 값을 반환 (자바 스크립트에서는 두 항을 같은 타입으로 변환 후 비교)
    - == : 동등 비교
    - === : 일치 비교
    - != : 부등 비교
    - !== 불일치 비교
...
/*
    대소 관계 비교 연산자 : 피연산자의 크기를 비교하여 논리 값으로 반환 (자바 스크립트에서는 두 항을 같은 타입으로 변환 후 비교)
    - > : 좌항이 우항보다 크다
    - < : 좌항이 우항보다 작다
    - >= : 좌항이 우항보다 크거나 같다
    - <= : 좌항이 우항보다 작거나 같다
...
result3 = number > 0 ? '양수' : number ? '음수' : '영';
